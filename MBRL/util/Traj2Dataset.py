...
class TrajDataset(data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any]:
        state = np.array(self.data[idx+1]['state'], dtype=np.float32)
        action = np.array(self.data[idx+1]['action'], dtype=np.float32)
        next_state = np.array(self.data[idx+1]['next_state'], dtype=np.float32)
